...
const uploadFilesAndCreateCourse = async (
    req: CustomRequest,
    res: Response,
) => {
    ...
    console.log('Uploading Thumbnail:', {
        buffer: thumbnail[0].buffer,
        folder: 'guided/thumbnails',
        type: 'image',
    });
    console.log('Uploading Video:', {
        buffer: videos[0].buffer,
        folder: 'guided/videos',
        type: 'video',
    });
    ...
    try {
        ...
        console.log(error);
        res.status(500).json({
            error: 'File upload or course creation failed',
        });
    }
};
